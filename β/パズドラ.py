...
max_count = 0
chainflag = [[0] * COL for _ in range(ROW)]  # インデックスを修正
dummy = [[0] * COL for _ in range(ROW)]      # インデックスを修正
...
def BEAM_SEARCH():
    ...
    for i in range(1, MAX_TURN):
        ...
        while que:
            temp = que.pop(0)
            for j in range(4):
                field_copy = copy.deepcopy(f_field)
                ...
                if 0 <= cand.nowC + dx[j] < COL and 0 <= cand.nowR + dy[j] < ROW:
                    ...
                    for k in range(100):
                        route[k][0] = cand.movei[k][0]
                        route[k][1] = cand.movei[k][1]
                    operation()
                    cand.score = sum_e()
                    cand.prev = j
                    pque.append(cand)
