...
# 判別したい色とそのRGB値
colors = {
    "赤": np.array([254, 178, 68]),
    "緑": np.array([102, 227, 82]),
    "青": np.array([75, 116, 255]),
    "黄": np.array([174, 255, 255]),
    "紫": np.array([105, 28, 66]),
...
for row in range(num_rows):
    ...
    for col in range(num_cols):
        ...
        # 中央の色を取得
        center_color = img_trimmed_part[row_height // 2, col_width // 2]
        classified_color = classify_color(center_color)

        # RGB値と判別結果を表示
        print(f'位置 ({row}, {col}) の中央色: RGB={center_color} -> 判別結果: {classified_color}')
